class TestConfigModel {
    ...
    factory TestConfigModel.fromJson(Map<String, dynamic> json){ 
        return TestConfigModel(
            testTypes: json["test_types"] == null ? [] : List<String>.from(json["test_types"]!.map((x) => x)),
            questionCounts: json["question_counts"] == null ? [] : List<int>.from(json["question_counts"]!.map((x) => x)),
            marksPerQuestion: json["marks_per_question"] == null ? [] : List<double>.from(json["marks_per_question"]!.map((x) => (x as num).toDouble()) ),
            negativeMarks: json["negative_marks"] == null ? [] : List<double>.from(json["negative_marks"]!.map((x) => (x as num).toDouble())),
            durations: json["durations"] == null ? [] : List<int>.from(json["durations"]!.map((x) => x)),
